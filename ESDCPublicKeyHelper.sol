...
// File: https://github.com/0xcyphered/secp256k1-solidity/blob/main/contracts/SECP256K1.sol
...
// File: SOULBOUNDNFTS/ESDCPublicKeyHelper.sol
...
contract PubKeyRecovery {
    /// @notice Recover the uncompressed secp256k1 public key (x||y) from a personal_sign’d message
    /// @param message  The 32-byte hash that was signed (before prefixed with "\x19Ethereum Signed Message:\n32")
    /// @param v        The recovery identifier (27 or 28)
    /// @param r        Signature r-value
    /// @param s        Signature s-value
    /// @return         64 bytes containing X (32) || Y (32)
    function recoverPubKey(
        ...
        // 1) Recompute the “Ethereum Signed Message” digest
        bytes32 prefixed = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32", message)
        );

        // 2) recover(x, y) expects:
        //    - digest as uint256
        //    - recovery id as 0 or 1  (so we subtract 27 from v)
        //    - r, s as uint256
        ...
        // 3) return the packed 64-byte public key
